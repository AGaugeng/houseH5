/*
 * @Author: Song
 * @Date: 2018-12-02 22:29:13
 * @LastEditors: Song
 * @LastEditTime: 2018-12-30 15:53:18
 * @Description: 付款查看房源详情
 */
...
import { getDetailPay } from "../../../actions/wechat"
import Toast from '../../../tool/toast'
class Counter extends Component {
    static propTypes = {
        getDetailPay: PropTypes.func.isRequired,
    ...
    passwordPay(password) {
        ...
        setTimeout(() => {
            this.props.getDetailPay([
                { name: 'pay_type', data: this.state.pay_type },
                { name: 'h_id', data: this.props.housepay.h_id },//
                { name: 'client', data: 1 },
    ...
    render() {
        ...
        const params = this.props.housepay
        const user = this.props.userinfo
        const item = this.props.token
        console.log(this.props.buyopenid)
        return (
            <Container style={styles.joinContainer}>
                <Header >
                    ...
                    <Body>
                        <Title style={{ color: '#000' }}>查看支付</Title>
                    </Body>
                    <Right />
                </Header>
                <Content>
                ...
                <Footer style={{ height: scaleSize(118), justifyContent: 'space-between' }}>
                    ...
                    <Text style={{ color: '#d6112d', marginLeft: scaleSize(30), lineHeight: scaleSize(118), marginRight: scaleSize(176), }}> <Text style={{ color: '#000' }}>总计：</Text>{params.pay_price}元</Text>
                        ...
                        <Button style={{ justifyContent: 'center', width: scaleSize(280), height: scaleSize(118), backgroundColor: '#e64e37' }}
                            ...
                            onPress={() => {
                                if (typeof plus === 'object') {
                                    this.appGetDetailPay(params)
                                } else {
                                    this.getDetailPay(params)
                                }
...
const mapStateToProps = state => ({
    ...
    housepay: state.house.housepay,
    openid: state.auth.buyopenid
})

export default connect(mapStateToProps, { getDetailPay })(Counter)
