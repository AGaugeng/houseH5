...
import {
    getCommunity,
    getHouseStruct, getHouseInfo, editResoldHouse, getHousePrice, addResoldHouse, getHouseType, UpdateSelectLabel, getHouseOrientation, getHouseAge, getHouseFloor, getHouseArea, getHouseUseRights, getHouseLayout, getHouseLabel, getHouseEquipment,
...
class Counter extends Component {
    static propTypes = {
        ...
        editResoldHouse: PropTypes.func.isRequired,
    ...
    componentDidMount() {
        setTimeout(() => {
            Object.keys(this.props.houseinfo).length > 0 && this.props.houseinfo.tag.split(',').length > 0 &&
                this.props.houseinfo.tag.split(',')//循环已有标签
                    .map((item1, index1) => {
    ...
    componentWillMount() {
        ...
        this.props.getHouseInfo([{ name: 'house_id', data: params.id }])
    ...
    editResoldHouse() {
        const { kitchen_num, balcony_num, property_age, title, struct, province, city, town, address, clicked, price, amountm, images,
        ...
        // console.log(this.props.filecallbacka)
        ...
        this.props.editResoldHouse([
            ...
            { name: 'price', data: price ? price : Math.round(item.price * 100 / 10000) / 100 },
            ...
            { name: 'property_use', data: property_use ? property_use : item.property_use },
            ...
            { name: 'property_age', data: property_age ? property_age : item.property_age },
    ...
    render() {
        const { province, city, town, address, community, kitchen_num, balcony_num, struct,
            floor, room_num, hall_num, bathroom_num, direction, elevator, decoration,
            construction_time,
            property_use } = this.state
        const item = this.props.houseinfo
        ...
        return (
            <Container >
                <Header style={{ borderBottomColor: '#E6E6E5', borderBottomWidth: scaleSize(1) }}>
                    ...
                    <Body>
                        <Title style={{ color: '#000' }}>售房修改</Title>
                    </Body>
                    <Right />
                ...
                {Object.keys(this.props.houseinfo).length === 0 ? <Spinner /> : <Content style={styles.container}>
                    <Form>
                            ...
                            <Col size={5} >
                                <Col>
                                    ...
                                    <Input
                                        ...
                                        placeholder={Math.round(item.price * 100 / 10000) / 100 + '万'}
                                        underlineColorAndroid="transparent"
                                ...
                                <Col >
                                    ...
                                    <View>
                                        ...
                                        <Text style={{ marginTop: scaleSize(30), color: '#ccc', fontSize: 14 }}>{!!property_use ? property_use : item.property_use}</Text>
                                    </View>
                                </Col >
                                {/* <Col >
                        ...
                        <ListItem onPress={() => this.props.navigation.navigate('ImageDetail', { image: item.images })} icon style={{ marginTop: scaleSize(38), flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Left>
                                <Text style={{
                                    color: '#000',
                                    fontSize: 14
                    ...
                    {this.state.isTrue ?
                        <TouchableOpacity transparent onPress={() => this.editResoldHouse()} >
...
const mapStateToProps = state => ({
    ...
    houseinfo: state.house.resoldchange,
